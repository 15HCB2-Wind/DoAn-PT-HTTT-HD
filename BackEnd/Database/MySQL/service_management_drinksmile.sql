...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 12, 2017 at 08:24 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `thongtinchinhanh` (`macn` VARCHAR(10))  BEGIN
    select *
    from
    (
    select chinhanh.machinhanh, chinhanh.tenchinhanh, chinhanh.diachi, Count(manhanvien)
    from chinhanh,nhanvien
    where chinhanh.machinhanh = nhanvien.machinhanh and chinhanh.machinhanh = macn
     and nhanvien.daxoa = false
    group by chinhanh.machinhanh
    ) a,
    (
    select Count(machuong)
    from chinhanh,chuongtrai
    where chinhanh.machinhanh = chuongtrai.machinhanh and chinhanh.machinhanh = macn
     and chuongtrai.daxoa = false
    group by chinhanh.machinhanh
    ) b,
    (
    select Count(makho)
    from chinhanh,khosua
    where chinhanh.machinhanh = khosua.machinhanh and chinhanh.machinhanh = macn
     and khosua.daxoa = false
    group by chinhanh.machinhanh
    ) c,
    (
    select Count(mabo)
    from chinhanh,bo,chuongtrai
    where chinhanh.machinhanh = chuongtrai.machinhanh and chinhanh.machinhanh = macn and
    chuongtrai.machuong = bo.machuong and bo.daxoa = false
    group by chinhanh.machinhanh
    ) d;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `thongtincongty` ()  BEGIN
    select *
    from 
    (
    select Count(areaid)
    ...
    (
    select Count(machinhanh)
    from chinhanh
    ) b,
    (
    select Count(manhacungcap)
    from nhacungcap
...
('HCMKS0001', 'Kho thường', 10000, 1426.179, '123 Trên mặt đất dươi bầu trời', b'0', 'Đang được sử dụng.', 'HCMCN001'),
('HCMKS0002', 'Kho lạnh - 1', 4000, 1000, 'Tìm được nói tao biết', b'0', 'Đang được sữa chửa.', 'HCMCN001'),
('HCMKS0003', 'Kho lạnh - 2', 5000, 0, 'Gần ngay trước mắt mà xa tận cùng trời', b'1', 'Ngưng hoạt động.', 'HCMCN001'),
('HCMKS0004', 'Kho thường', 10000, 604.72, 'Giỏi tìm tao đi', b'0', 'Đang được sử dụng.', 'HCMCN002'),
('HCMKS0005', 'Kho lạnh', 5000, 3000, 'You cant see me \\m/', b'0', 'Đang được sử dụng.', 'HCMCN002'),
('HNKS0001', 'Kho thường', 10000, 161.08, 'Everywhere you know', b'0', 'Đang được sử dụng.', 'HNCN001'),
('HNKS0002', 'Kho lạnh', 5000, 1000, 'Vũ trụ song song', b'0', 'Đang được sử dụng.', 'HNCN001');
